...
export default function ResultReportPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/settings").then((r) => r.json()).then((j) => setSetting(j.setting)).catch(() => {});
  }, []);
    ...
    <>
      ...
      <div className="no-print mb-5 flex flex-wrap items-center justify-between gap-3">
        <Button asChild variant="ghost" size="sm" className="gap-2">
          <Link href="/admin"><ArrowLeft className="size-4" /> Back to overview</Link>
        </Button>
      ...
      <div className="report-sheet mx-auto max-w-[820px] rounded-lg border border-border bg-white p-10 text-[#1a1a1a] shadow-sm">
        ...
        <header className="border-b-2 border-[#0e5a37] pb-5 text-center">
          ...
          <p className="mt-1 text-sm font-semibold">{setting.faculty}</p>
          <p className="text-sm">{setting.department}</p>
          <h2 className="mt-4 text-base font-bold">{setting.electionTitle}</h2>
